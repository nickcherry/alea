...
Concrete example with 35-min lead time:
...
[HH-1:00 ... HH:00]    [HH:00 ............ HH+1:00]   [HH+1:00 ........ HH+2:00]
                       ...
                       (HH:25, 25 min into now,       (HH+1:00, 35 min after
                        35 min before target opens)    decision fires)
```

- `target.open` = HH+1:00
- `decisionTsMs = target.open - leadTime` (e.g. HH:25 for 35-min lead)
...
For the current canonical setup (period = `1h`, leadTime = 35 min):

| symbol | value |
|---|---|
| `period` | 60 min |
| `leadTime` | 35 min |
| `decisionTsMs - target.open` | -35 min |
| `decisionTsMs - now.open` | +25 min |
| `now` partial 1m window | `[now.open, decisionTsMs]`, ~25 min |
...
- [ ] CLI docstrings in `src/bin/research/*Sweep.ts` describe this
      ...
      "HH:50" — say "35 minutes before target opens" / "the now candle
