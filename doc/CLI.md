...
- `dry:*`
  `dry:run` — long-running process that refreshes Pyth plus Coinbase spot candles one full candle before target open, classifies the regime from closed bars, runs the rostered committee for `5m,15m` by default (override with `--periods`), persists decisions to `dry_run_decisions`, and tracks the configured simulated Polymarket order fill status. See [DRY_RUN.md](./DRY_RUN.md).
- `dashboards:*`
