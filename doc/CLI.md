...
- `dry:*`
  `dry:run` — long-running process that streams live Pyth ticks, classifies the current regime, runs the rostered committee at every 5m boundary, persists decisions to `dry_run_decisions`, and tracks the configured simulated Polymarket order fill status. See [DRY_RUN.md](./DRY_RUN.md).
- `dashboards:*`
