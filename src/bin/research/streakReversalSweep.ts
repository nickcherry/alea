...
import {
  detectStreakReversalAt,
  type StreakReversalBaseConfig,
  type StreakReversalTrigger,
...
type SweepConfig = StreakReversalBaseConfig & ThesisLifecycleConfig;

const streakLengthValues = [3, 4, 5, 6, 8, 10] as const;
const minTotalReturnPctValues = [0, 0.005, 0.01, 0.02] as const;
...
const candidateCount =
  streakLengthValues.length *
  minTotalReturnPctValues.length *
...
export const researchStreakReversalSweepCommand = defineCommand({
  name: "research:streak-reversal-sweep",
  summary: "Sweep 1h streak-reversal candidates",
  description:
    "Runs a local research sweep for the Streak Reversal filter on 1h markets. Simplest mean-reversion filter: after N consecutive same-direction closed bars and a minimum cumulative return, bet the opposite direction. Bets against synth direction by construction.",
  options: [
    defineValueOption({
      key: "assets",
      long: "--assets",
  ...
  examples: [
    "bun alea research:streak-reversal-sweep",
    "bun alea research:streak-reversal-sweep --assets btc,eth",
  ],
  output:
  ...
  async run({ io, options }) {
    ...
    io.writeStdout(
      `${pc.bold("research:streak-reversal-sweep")} ${pc.dim(`${new Date(options.start).toISOString()} -> ${new Date(options.end).toISOString()}`)} ${pc.dim(`assets=${assets.join(",")}`)} ${pc.dim(`candidates=${candidateCount}`)}\n`,
    );
...
async function runSweep({
  ...
  const artifact = {
    ...
    objective:
      "Sweep Streak Reversal candidates on 1h markets — simplest mean-reversion filter.",
    outcomeSource:
  ...
  const outPath = writeSweepArtifact({
    slug: "one-hour-streak-reversal-sweep",
    payload: artifact,
  });
  return {
    outPath,
...
function evaluateTarget({
  ...
  for (const streakLength of streakLengthValues) {
    for (const minTotalReturnPct of minTotalReturnPctValues) {
      const triggers: {
        readonly trigger: StreakReversalTrigger;
        readonly barsAgo: number;
      }[] = [];
      const baseConfig: StreakReversalBaseConfig = {
        streakLength,
        minTotalReturnPct,
        maxSignalAgeBars: maxAgeWindow,
      };
      for (let i = earliest; i <= lastIndex; i += 1) {
        const trigger = detectStreakReversalAt({
          bars,
          index: i,
          config: baseConfig,
        });
...
function configKey(config: SweepConfig): string {
  return [
    `streak=${config.streakLength}`,
    `ret=${config.minTotalReturnPct}`,
