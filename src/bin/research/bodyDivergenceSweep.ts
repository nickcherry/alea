...
import { bodyDivergenceStructuralCheck } from "@alea/lib/filters/bodyDivergence";
import {
...
type SweepConfig = BodyDivergenceBaseConfig & ThesisLifecycleConfig;
...
const minBodyShrinkPctValues = [0.2, 0.4, 0.6, 0.8] as const;
...
export const researchBodyDivergenceSweepCommand = defineCommand({
  name: "research:body-divergence-sweep",
  summary: "Sweep 1h body-divergence candidates",
  description:
    "Runs a local research sweep for the Body Divergence filter on 1h markets. Trigger compares bar bodies at consecutive same-direction pivots; if a new pivot extreme prints with a meaningfully smaller body than the prior pivot, votes the reversal direction.",
  options: [
    defineValueOption({
      key: "assets",
      long: "--assets",
  ...
  examples: [
    "bun alea research:body-divergence-sweep",
    "bun alea research:body-divergence-sweep --assets btc,eth",
  ],
  output:
  ...
  async run({ io, options }) {
    ...
    io.writeStdout(
      `${pc.bold("research:body-divergence-sweep")} ${pc.dim(`${new Date(options.start).toISOString()} -> ${new Date(options.end).toISOString()}`)} ${pc.dim(`assets=${assets.join(",")}`)} ${pc.dim(`candidates=${candidateCount}`)}\n`,
    );
...
async function runSweep({
  ...
  const artifact = {
    ...
    objective:
      "Sweep Body Divergence candidates on 1h markets — divergence variant using bar body as the momentum proxy.",
    outcomeSource:
  ...
  const outPath = writeSweepArtifact({
    slug: "one-hour-body-divergence-sweep",
    payload: artifact,
  });
  return {
    outPath,
...
function evaluateTarget({
  ...
  for (const pivotBars of pivotBarsValues) {
    for (const range of rangePairs) {
      for (const minBodyShrinkPct of minBodyShrinkPctValues) {
        for (const maxSignalAgeBars of maxSignalAgeBarsValues) {
          const baseConfig: BodyDivergenceBaseConfig = {
            leftBars: pivotBars,
            rightBars: pivotBars,
          ...
          const match = findRecentBodyDivergence({ bars, config: baseConfig });
          if (!match.matched) {
            continue;
          }
...
function configKey(config: SweepConfig): string {
  return [
    ...
    `shrink=${config.minBodyShrinkPct}`,
