...
const leadMinutes = Math.round(
  tradeDecisionLeadTimeMs({ period: "1h" }) / 60_000,
);
...
export const researchExtensionReversalSweepCommand = defineCommand({
  name: "research:extension-reversal-sweep",
  summary: "Sweep 1h extension-reversal candidates",
  description:
    "Runs a local research sweep for the Extension Reversal filter on 1h markets. The filter fades a compounded extension: when the synth (in-progress) bar and the prior closed bar both push the same direction with returns above their respective thresholds, the filter bets the opposite direction.",
  options: [
    defineValueOption({
      key: "assets",
      long: "--assets",
  ...
  examples: [
    "bun alea research:extension-reversal-sweep",
    "bun alea research:extension-reversal-sweep --assets btc,eth",
  ],
  output:
  ...
  async run({ io, options }) {
    ...
    io.writeStdout(
      `${pc.bold("research:extension-reversal-sweep")} ${pc.dim(`${new Date(options.start).toISOString()} -> ${new Date(options.end).toISOString()}`)} ${pc.dim(`assets=${assets.join(",")}`)} ${pc.dim(`candidates=${candidateCount}`)}\n`,
    );
...
async function runSweep({
  ...
  const artifact = {
    ...
    objective:
      "Sweep Extension Reversal candidates on 1h markets — fade compounded synth+last-closed extensions.",
    outcomeSource:
      "Pyth spot 1h candle direction. This does not include Polymarket market prices or odds.",
    decisionTiming: `For each 1h target candle, decide ${leadMinutes} minutes before the target candle opens using a synthetic of the prior (in-progress) hour built from 1m Pyth candles through \`target.open - ${leadMinutes}min\`. The filter never sees any data from the target candle itself.`,
    startMs,
    endMs,
    assets,
    timeframe: "1h",
  ...
  const outPath = writeSweepArtifact({
    slug: "one-hour-extension-reversal-sweep",
    payload: artifact,
  });
  return {
    outPath,
...
function evaluateTarget({
  ...
  const earliest = Math.max(1, lastIndex - maxAgeWindow);
