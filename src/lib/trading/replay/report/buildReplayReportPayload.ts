...
export function buildReplayReportPayload({
  records,
  sourcePath,
  ...
  let config: ReplayReportConfig | null = null;
  const parseErrors: string[] = [];
  const ordersById = new Map<string, ReplayReportOrder>();
  const windowsByStart = new Map<number, ReplayWindowSummary>();
  ...
  for (const [index, recordUnknown] of records.entries()) {
    ...
    if (type === "window_checkpoint" || type === "window_finalized") {
      ...
      const orders = arrayField({ object: record, key: "orders" })
        ...
        .filter((order): order is ReplayReportOrder => order !== null);
...
function summarizeByAsset({
  ...
  readonly config: ReplayReportConfig | null;
}): ReplayAssetSummary[] {
