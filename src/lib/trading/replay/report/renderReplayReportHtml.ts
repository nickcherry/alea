...
<head>
  ...
  <title>Alea &middot; Replay Session</title>
...
<body>
  <div class="alea-shell">
    <header class="alea-header">
      ...
      <h1 class="alea-title">Replay Session</h1>
    </header>
...
function renderExecutionComparisonSection({
  ...
  readonly summary: ReplayReportSummary;
  readonly orders: readonly ReplayReportOrder[];
}): string {
                ...
                ${metricRow({
                  ...
                  valueClass:
                    summary.officialProxyDisagreementCount > 0
                      ? "alea-num-negative"
                      : "",
                  note: "Polymarket settle vs Chainlink reference",
                })}
              </tbody>
            </table>
          </div>
...
function renderPlacedCounterfactualSection({
  ...
  readonly summary: ReplayReportSummary;
  readonly orders: readonly ReplayReportOrder[];
  readonly unfilledOrders: readonly ReplayReportOrder[];
}): string {
...
function renderAssetTable({
  ...
  readonly assets: readonly ReplayAssetSummary[];
}): string {
  return `
...
function renderWindowTable({
  ...
  readonly windows: readonly ReplayWindowSummary[];
}): string {
  return `
    <section class="alea-card">
      <div class="alea-card-header">
...
function renderAssetRow({
  ...
  readonly asset: ReplayAssetSummary;
}): string {
  return `
    <tr>
...
function renderWindowRow({
  ...
  readonly window: ReplayWindowSummary;
}): string {
...
function renderFilledOrderRow({
  ...
  readonly order: ReplayReportOrder;
}): string {
  return `
    <tr>
...
function orderTime({ order }: { readonly order: ReplayReportOrder }): string {
  return `<div class="alea-mono alea-nowrap">${formatTime({
    ms: order.placedAtMs,
  })}</div><div class="order-id alea-mono">${escapeHtml(order.id)}</div>`;
}

function outcomePill({ order }: { readonly order: ReplayReportOrder }): string {
  const outcomeClass =
...
function modelCell({ order }: { readonly order: ReplayReportOrder }): string {
...
function isFinalized(order: ReplayReportOrder): boolean {
  return order.officialOutcome !== null;
}

function isActuallyFilled(order: ReplayReportOrder): boolean {
